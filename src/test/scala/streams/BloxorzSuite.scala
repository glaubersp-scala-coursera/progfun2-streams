...
class BloxorzSuite {
  ...
  @Test def `neighborsWithHistory`: Unit = new Level1 {
    assertEquals(
  ...
  @Test def `newNeighborsOnly`: Unit = new Level1 {
    assertEquals(
      newNeighborsOnly(
...
//  @Rule def individualTestTimeout = new org.junit.rules.Timeout(10000000 * 1000)
